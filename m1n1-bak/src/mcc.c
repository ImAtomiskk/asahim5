...
int mcc_init_t8142(int *path, u32 reg_offset, u32 plane_count, u32 dcs_count,
                   struct tz_regs *tz_regs)
{
    for (int i = 0; i < mcc_count; i++) {
        u64 base;
...
int mcc_init_m3(int node, int *path)
{
    ...
    int ret = -1;
    if (adt_is_compatible(adt, node, "mcc,t8122") || adt_is_compatible(adt, node, "mcc,t8142"))
...
int mcc_init_m5(int node, int *path)
{
    u64 amcc_count;
    if (ADT_GETPROP(adt, node, "amcc_aperture_count", &amcc_count) < 0 &&
        ADT_GETPROP(adt, node, "amcc-count", &amcc_count) < 0) {
    ...
    printf("MCC: Initializing M5 MCCs (%d instances)...\n", chip_id, mcc_count);

    int ret = -1;
    if (adt_is_compatible(adt, node, "mcc,t8142")) {
        int reg_offset = 7;
        ret = mcc_init_t8122(path, reg_offset, plane_count, dcs_count, &t8142_tz_regs);
    } else {
        printf("MCC: Unsupported version:%s\n", adt_get_property(adt, node, "compatible")->value);
    }
...
int mcc_init(void)
{
    ...
    if (adt_is_compatible(adt, node, "mcc,t8103")) {
        ...
        return mcc_init_m3(node, path);
    } else if (adt_is_compatible(adt, node, "mcc,t8142")) {
        return mcc_init_m5(node, path);
    } else {
        printf("MCC: Unsupported version: %s\n", adt_get_property(adt, node, "compatible")->value);
        printf("MCC: Updated 12.09.2026");
        return -1;
    }
}
